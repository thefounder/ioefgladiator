...
void CalculateRanks( void ) {
	...
	sv.follow1 = -1;
	sv.follow2 = -1;
	sv.numConnectedClients = 0;
	sv.numNonSpectatorClients = 0;
	sv.numPlayingClients = 0;
	sv.numVotingClients = 0;		// don't count bots
	
	
	
	for (i = 0; i < ARRAY_LEN(sv.numteamVotingClients); i++)
		sv.numteamVotingClients[i] = 0;

	for ( i = 0 ; i < sv_maxclients->integer ; i++ ) {
		if ( svs.clients[i].state != CS_ZOMBIE ) {
			sv.sortedClients[sv.numConnectedClients] = i;
			sv.numConnectedClients++;
			ps = SV_GameClientNum( i );
			if ( ps->persistant[4]!= TEAM_SPECTATOR ) {
				sv.numNonSpectatorClients++;
			
				// decide if this should be auto-followed
				if ( svs.clients[i].state == CS_ACTIVE ) {
					sv.numPlayingClients++;
					if ( !(svs.clients[i].gentity->r.svFlags & SVF_BOT) ) {
						sv.numVotingClients++;
						if ( ps->persistant[4] == TEAM_RED )
							sv.numteamVotingClients[0]++;
						else if ( ps->persistant[4]== TEAM_BLUE )
							sv.numteamVotingClients[1]++;
					}
					
					if ( sv.follow1 == -1 ) {
						sv.follow1 = i;
					} else if ( sv.follow2 == -1 ) {
						sv.follow2 = i;
					}
				}
			}
...
void CheckVote(void) {
	...
	//terminates vote when game enters intermission (due to vote string vanishing)
	for(i=0; i < sv_maxclients->integer; i++ ){
		ps = SV_GameClientNum( i );
		if( ps->pm_type == PM_INTERMISSION){
			SV_SendServerCommand( NULL, "print \"^1Vote failed.\n\"");
			sv.voteTime=0;
			return;
		}
	}
	//terminates vote when game goes from warmup to action
	if(sv.warmupcounter==1){
		SV_SendServerCommand( NULL, "print \"^1Vote failed.\n\"");
		sv.voteTime=0;
		return;
	}
	///////
	if ( sv.time - sv.voteTime >= VOTE_TIME ) {
		SV_SendServerCommand( NULL, "print \"^1Vote failed.\n\"" );
	} else {
		if ( sv.voteYes > sv.numVotingClients/2 ) {
			// execute the command, then remove the vote
			if (sv.map_restart==qfalse && !Q_stricmp( sv.arg1copy, "map_restart" ) ){
				SV_SendServerCommand( NULL, "print \"^2Vote passed.\n\"" );
				Cbuf_ExecuteText( EXEC_APPEND, va("%s\n", sv.voteString ) );
				sv.map_restart=qtrue;
			}else if (sv.match_restart==qfalse && !Q_stricmp( sv.arg1copy, "match_restart" ) ){
				SV_SendServerCommand( NULL, "print \"^2Vote passed.\n\"" );
				Cbuf_ExecuteText( EXEC_APPEND, va("%s\n", sv.voteString ) );
				sv.match_restart=qtrue;
			}else if (!Q_stricmp( sv.arg1copy, "mapenable" )){
				SV_SendServerCommand( NULL, "print \"^2Vote passed.\n\"" );
				SV_SendServerCommand( NULL, "print \"^2Callvote map is now enabled for the duration of this map.\n\"" );
			}else{
			SV_SendServerCommand( NULL, "print \"^2Vote passed.\n\"" );
			if(Q_stricmp(sv.nextmapvoteString, "" ) ){
				SV_SendServerCommand( NULL, "print \"Next map (for the next match) will be: %s\n\"", sv.nextmapname);
				Cbuf_ExecuteText( EXEC_APPEND, va("%s\n", sv.nextmapvoteString ) );
				Q_strncpyz(sv.nextmapvoteString, "", sizeof(sv.nextmapvoteString));
				sv.avotehasalreadypassed=qtrue;
			}
			Cbuf_ExecuteText( EXEC_APPEND, va("%s\n", sv.voteString ) );
			}
		}else if ( sv.voteNo >= sv.numVotingClients/2 ) {
...
void SV_Frame( int msec ) {
	...
	//warmupvoting variable
	SV_GetConfigstring( CS_WARMUP, warmupstring, sizeof(warmupstring));
	sv.warmupTime = atoi(warmupstring);

	//terminates vote after initial warmup (due to vote string vanishing)
	if(sv.warmupTime>0){
		sv.warmupcounter=0;
	}
	
	if(sv.warmupTime==0&&sv.warmupcounter<10){
		++sv.warmupcounter;
	}
